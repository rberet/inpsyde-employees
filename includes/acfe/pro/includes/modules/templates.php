...
class acfe_dynamic_templates extends acfe_dynamic_module
{
     ...
     * Validate Values
     */
    public function validate_values($valid, $value, $field, $input)
    {
     ...
     * Add Metaboxes
     */
    public function add_metaboxes($post_type, $post, $field_groups)
    {
     ...
     */
    public function render_meta_box_rules($post, $metabox)
    {
     ...
     */
    public function render_meta_box_instructions($post, $metabox)
    {
        ?>
        <div class="acf-field">
            <div class="acf-label">
     ...
     * Save
     */
    public function save($name, $post_id)
    {
     ...
     * Location: Operators
     */
    public function location_operators($operators, $rule)
    {
        $operators = array(
            '==' => __("is equal to", 'acf'),
        );
     ...
     */
    public function location_match_target($match, $rule, $screen, $field_group)
    {
     ...
     * Match: Template
     */
    public function location_match_template($match, $rule, $screen, $field_group)
    {
        $post_type = acf_maybe_get($screen, 'post_type');
        $template_id = acf_maybe_get($screen, 'post_id');
        $template_id = $this->validate_post_id($template_id);
        
        if (!$template_id || $post_type !== $this->post_type || $field_group['key'] === 'group_acfe_dynamic_template_side') {
            return $match;
        }
     ...
     */
    public function local_template_load_target()
    {
        ...
        // User Edit
        } elseif (in_array($pagenow, array('profile.php', 'user-edit.php'))) {
            global $user_id;
        ...
        // User New
        } elseif (in_array($pagenow, array('user-new.php'))) {
            $screen = array(
                'user_id'   => 'new',
                'user_form' => 'add'
            );
        ...
        foreach ($templates as $template) {
            ...
            // Match screen
            if (!$this->local_template_match_location($template, $screen)) {
                continue;
            }
    ...
    /*
     * Local Template: Match Screen
     */
    public function local_template_match_location($template, $args)
    {
        ...
        if ($template['location']) {
            ...
            // Loop through location groups.
            foreach ($template['location'] as $group) {
                ...
                foreach ($group as $rule) {
                    if (!acf_match_location_rule($rule, $screen, $template)) {
                        $match_group = false;
                        break;
                    }
                }
     ...
     * Apply Values
     */
    public function apply_values($template)
    {
     ...
     */
    public function get_target_locations($field_groups, $post_id)
    {
        $groups = array();
